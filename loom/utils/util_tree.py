...
def new_node(node_id=None, text="", mutable=True):
    ...
    node = {"id": node_id, "text": text, "children": [], "mutable": mutable}
    return node
...
def generate_conditional_tree(root, filter=None):
    return {
        d["id"]: d
        for d in flatten_tree(
            tree_subset(root=root, filter=filter),
        )
    }
...
def overwrite_subtree(node, attribute, new_value, old_value=None, force_overwrite=False):
    if (
        force_overwrite
        or (attribute not in node)
        or old_value is None
        or (node[attribute] == old_value)
...
def search(
    ...
    matches = []
    if not (text or tags) or (filter_set is not None and root["id"] not in filter_set) or max_depth == 0:
        return []
    if text:
        matches_iter = (
            re.finditer(pattern, root[text_attribute_name])
            if case_sensitive
            else re.finditer(pattern, root[text_attribute_name], re.IGNORECASE)
        )
        for match in matches_iter:
            matches.append({"node_id": root["id"], "span": match.span(), "match": match.group()})
    if tags:
