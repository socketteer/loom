...
from loom.utils.util_tree import (
    collapsed_wavefunction,
    flatten_tree,
    limited_branching_tree,
    limited_distance_tree,
    tree_subset,
)
...
class Paint(Module):
    ...
    def build(self, parent):
        ...
        self.undo_button = tk.Label(
            self.root, image=icons.get_icon("undo-white", size=20), bg=bg_color(), cursor="hand2"
        )
        ...
        self.brush_button = tk.Label(
            self.root, image=icons.get_icon("brush-white", size=20), bg=bg_color(), cursor="hand2"
        )
        self.brush_button.grid(row=0, column=1)
        self.brush_button.bind("<Button-1>", self.use_brush)

        self.eraser_button = tk.Label(
            self.root, image=icons.get_icon("eraser-white", size=20), bg=bg_color(), cursor="hand2"
        )
        self.eraser_button.grid(row=0, column=2)
        self.eraser_button.bind("<Button-1>", self.use_eraser)

        self.current_color = tk.Label(
            self.root, image=icons.get_icon("eyedropper-white"), fg=text_color(), cursor="hand2", bd=3
        )
        ...
        self.past_opacity_slider = tk.Scale(
            self.root, from_=0, to=1, orient="horizontal", resolution=0.1, variable=self.past_recursive_veil_opacity
        )
        ...
        self.add_layer_button = tk.Label(
            self.layers_frame, image=icons.get_icon("plus-lightgray", size=20), bg=bg_color(), cursor="hand2"
        )
    ...
    def open(self, transparent=True, *args):
        ...
        filename = filedialog.askopenfilename(
            initialdir=media_dir, title="Select file", filetypes=(("png files", "*.png"), ("all files", "*.*"))
        )
        if filename:
            # print(filename)
...
class Children(Module):
    def __init__(self, callbacks, state):
        ...
        self.children = NodeWindows(
            callbacks,
            buttons=["close", "go", "edit", "archive", "delete"],
    ...
    def build(self, parent):
        ...
        self.add_child_button = tk.Label(
            self.menu_frame, image=icons.get_icon("plus-lightgray"), bg=bg_color(), cursor="hand2"
        )
...
class ReadChildren(Module):
    ...
    def add_child(self, node):
        ...
        child_label = tk.Label(
            ...
            justify="left",
            font=("Georgia", 12),
            image=icons.get_icon("arrow-white", 16),
            compound=tk.LEFT,
            padx=10,
        )
    ...
    def refresh(self):
        if self.scroll_frame:
            children_options = (
                self.callbacks["Get children"]["callback"](filter=self.filter()) if self.show_options() else []
            )
    ...
    def context_menu(self, event, node):
        ...
        menu.add_command(
            ...
            command=lambda: self.callbacks["Edit in module"]["callback"](node=node, create_attribute="child_preview"),
        )
...
class JanusPlayground(Module):
    ...
    def build(self, parent):
        ...
        self.generate_button = ttk.Button(
            self.button_frame,
            text="Insert prompt",
            image=icons.get_icon("rightarrow-lightgray"),
            cursor="hand2",
            command=self.insert_prompt,
            compound="left",
        )
        ...
        self.generate_button = ttk.Button(
            self.button_frame,
            text="Generate",
            image=icons.get_icon("brain-blue"),
            cursor="hand2",
            command=lambda: self.generate(mode="completions"),
            compound="left",
        )
        self.generate_button.pack(side="left", expand=True)
        self.eval_prompt_button = ttk.Button(
            self.button_frame,
            text="Evaluate prompt",
            image=icons.get_icon("chart-blue"),
            cursor="hand2",
            command=lambda: self.generate(mode="eval"),
            compound="left",
        )
        self.eval_prompt_button.pack(side="left", expand=True)
        self.export_button = ttk.Button(self.button_frame, text="Export", cursor="hand2", command=self.export)
        self.export_button.pack(side="left", expand=True)
        self.settings_button = tk.Label(
            self.button_frame, image=icons.get_icon("settings-lightgray"), bg=bg_color(), cursor="hand2"
        )
    ...
    def toggle_settings(self, *args):
        if not self.settings_control:
            ...
            self.settings_control = GenerationSettings(
                orig_settings=self.state.generation_settings, realtime_update=True
            )
            ...
            self.inline_settings_control = GenerationSettings(
                orig_settings=self.state.inline_generation_settings, realtime_update=True
            )
...
class MiniMap(Module):
    ...
    def build(self, parent):
        ...
        self.minimap_settings = MinimapSettings(
            orig_params=self.state.module_settings["minimap"],
    ...
    def refresh(self):
        ...
        if self.settings()["prune_mode"] == "ancestry_dist":
            pruned_tree = limited_branching_tree(
                self.ancestry, filtered_tree, depth_limit=self.settings()["path_length_limit"]
            )
        elif self.settings()["prune_mode"] == "selection_dist":
            pruned_tree = limited_distance_tree(
                filtered_tree,
                self.selected_node,
                distance_limit=self.settings()["path_length_limit"],
            ...
            pruned_tree = collapsed_wavefunction(
                self.ancestry, filtered_tree, self.selected_node, depth_limit=self.settings()["path_length_limit"]
            )
            center_subtree = True
        elif self.settings()["prune_mode"] == "in_nav":
            pruned_tree = filtered_tree
        elif self.settings()["prune_mode"] == "open_in_nav":
            pruned_tree = tree_subset(
                filtered_tree,
                filter=lambda node: self.state.is_root(node)
                or self.callbacks["Node open"]["callback"](node=self.state.parent(node)),
            )
        else:
            pruned_tree = filtered_tree
    ...
    def draw_precomputed_tree(self, root):
        ...
        for child in root["children"]:
            ...
            self.draw_connector(
                ...
                child_y,
                fill="#000000",
                width=self.settings()["line_thickness"],
    ...
    def move_nodes(self):
        added_ids, deleted_ids = react_changes(
            old_components=self.old_node_coords.keys(), new_components=self.node_coords.keys()
        )
            ...
            for l in remaining_levels:
                ...
                for node_id in l:
                    self.node_coords[node_id] = (
                        self.node_coords[node_id][0] - offset - x_mid,
...
class Input(Module):
    ...
    def build(self, parent):
        ...
        self.submit_button = ttk.Button(
            self.button_frame, text="Submit", image=icons.get_icon("arrow-white"), compound="right", command=self.submit
        )
    ...
    def submit(self):
        ...
        self.callbacks["Submit"]["callback"](
            text=modified_text, auto_response=self.settings().get("auto_response", True)
        )
...
class Edit(Module):
    ...
    def build(self, parent):
        ...
        self.template_checkbox = tk.Checkbutton(
            ...
            text="Template",
            variable=self.template_bool,
            onvalue=True,
            offvalue=False,
            command=self.write_template,
        )
        ...
        self.add_text_attribute_button = tk.Label(
            self.frame, text="Add Text Attribute", cursor="hand2", fg=text_color(), bg=bg_color(), relief="raised"
        )
    ...
    def template_preset_changed(self, *args):
        ...
        if template == "none":
            ...
            self.text_attributes["text"].textbox.insert(
                tk.END,
    ...
    def rebuild_textboxes(self):
        ...
        self.node = (
            self.state.node(self.settings()["node_id"]) if self.settings()["node_id"] else self.state.selected_node
        )
        ...
        self.text_attributes["text"] = TextAttribute(
            master=self.textboxes_frame,
            attribute_name="text",
        ...
        if "text_attributes" in self.node:
            for attribute in self.node["text_attributes"]:
                self.text_attributes[attribute] = TextAttribute(
                    master=self.textboxes_frame,
                    attribute_name=attribute,
                    read_callback=lambda attribute=attribute: self.callbacks["Get text attribute"]["callback"](
                        attribute=attribute, node=self.node
                    ),
                    write_callback=lambda text, attribute=attribute: self.save_text_attribute(
                        attribute_name=attribute, text=text
                    ),
    ...
    def new_attribute(self, name):
        ...
        self.text_attributes[name] = TextAttribute(
            master=self.textboxes_frame,
            attribute_name=name,
...
class Transformers(Module):
    ...
    def build(self, parent):
        ...
        self.template_editor = TextAttribute(
            master=self.frame,
        ...
        self.prompt_literal_textbox = TextAware(
            self.prompt_frame.collapsable_frame, bd=2, height=3, undo=True, relief="raised"
        )
        ...
        self.generation_settings_dashboard = SpecialGenerationSettings(
            orig_params=self.generation_settings, state=self.state, realtime_update=True, parent_module=self
        )
        ...
        self.generate_button = ttk.Button(
            self.frame, text="Generate", image=icons.get_icon("brain-blue"), compound="left", command=self.generate
        )
    ...
    def open_template(self, template):
        ...
        for input in template["inputs"]:
            self.input_editors[input] = TextAttribute(
    ...
    def load_template(self):
        file_path = filedialog.askopenfilename(
            initialdir="./loom/_config/transformers",
    ...
    def save_template(self):
        ...
        file_path = filedialog.asksaveasfilename(
            initialdir="./loom/_config/transformers", title="Save prompt template", filetypes=[("Json files", ".json")]
        )
...
class GenerationSettings(Module):
    ...
    def build(self, parent):
        Module.build(self, parent)
        self.settings_control = FullGenerationSettings(
...
class FrameEditor(Module):
    ...
    def build(self, parent):
        Module.build(self, parent)
        self.frame_editor = TextAttribute(
            master=self.frame,
        ...
        self.state_viewer = TextAttribute(
            master=self.frame, attribute_name="state", bd=2, max_height=30, expand=True, relief="raised"
        )
        ...
        self.write_to_user_frame_button = tk.Button(
            self.frame, text="Copy to user frame", command=self.write_to_user_frame
        )
    ...
    def get_presets(self):
        ...
        with open("./loom/_config/interfaces/interfaces.json") as f:
...
class Memories(Module):
    ...
    def build(self, parent):
        Module.build(self, parent)
        self.memory_editor = AttributesEditor(
            attribute_category="memory",
...
class Vars(Module):
    ...
    def build(self, parent):
        Module.build(self, parent)
        self.vars_editor = AttributesEditor(
...
class Wavefunction(Module):
    ...
    def build(self, parent):
        ...
        self.propagate_button = ttk.Button(
            self.buttons_frame, text="Propagate", compound="right", command=self.propagate
        )
        ...
        self.reset_zoom_button = ttk.Button(
            self.buttons_frame, text="Reset zoom", compound="right", command=self.reset_zoom
        )
        self.reset_zoom_button.pack(side="left")
        self.add_path_button = ttk.Button(
            self.buttons_frame, text="Add path to tree", compound="right", command=self.add_path
        )
        self.add_path_button.pack(side="left")
        self.save_image_button = ttk.Button(
            self.buttons_frame, text="Save image", compound="right", command=self.save_image
        )
    ...
    def propagate(self):
        if self.wavefunction.active_wavefunction():
            ...
            multiverse, ground_truth, prompt = self.state.generate_greedy_multiverse(
        ...
        self.wavefunction.draw_multiverse(
            multiverse=multiverse, ground_truth=ground_truth, start_position=start_position, prompt=prompt
        )
